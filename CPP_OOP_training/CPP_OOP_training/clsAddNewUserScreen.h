...
public :
	...
	static void Add_New_User() {
		...
		Draw_Header("   Add New User");

		cout << "Enter User Name : " << endl;
		string UserName; 
		UserName = clsInputValidate::Read_String();
		while (clsUser::Is_Exist(UserName)) {
			cout << "This User Name Already Exist, Try Another Name : "; 
			UserName = clsInputValidate::Read_String();
		}

		clsUser user = clsUser::Get_New_User_Object(UserName);
		cout << "----------------------------------" << endl;
		...
		if (save_result == clsUser::enSave_result::Added_Successfully) {
			cout << "Account Added Successfully :-)" << endl << endl;
			_Print_User(user);
		}
		else {
			cout << "Error ! , User does't saved because it's empty" << endl;
		}
